...
| Container workspace needs attention | The engine may already be installed, but the container cannot read/write GUIDE-IEI's working folder (normally `~/.iei-variant-review/container-work`). Setup can back up and repair the selected Colima profile's sharing while preserving existing settings. If other containers are running, finish those workloads, then choose **Retry preparation**. Docker Desktop users may need to allow the folder named in the error in **Settings → Resources → File sharing**. The app does not require sharing `/Applications`. Selected datasets and external drives still need their own access checks; no dataset deletion or VEP rebuild is needed. |
