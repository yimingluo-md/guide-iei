...
- **Free and open source** — MIT-licensed, with no subscription or
  per-sample cost.
...
- **Designed for clinicians and wet-lab scientists** — dataset setup written
  ...
  datasets.
- **In-depth loss-of-function curation** — LOFTEE, augmented with
  recalculation of the 50-bp rule at the premature termination codon
  generated by each frameshift, and haplotype-aware recognition of
  frame-restoring indel pairs.
- **Non-coding variant prioritization** — a whole-genome review path
  practical on a standard workstation, reaching beyond the exome: SpliceAI
  for deep intronic splice-altering variants, PromoterAI for promoter
  variants, and regulatory context from ENCODE cCREs with curated tissue and
